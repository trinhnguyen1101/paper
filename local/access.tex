...
\DeclareRobustCommand{\textbf}[1]{{\bfseries #1}}
\DeclareRobustCommand{\bf}{\bfseries}
\def\@doi{}
\makeatother
% --- end patch ---
...
\author{\uppercase{Luong Thien Phuc}\authorrefmark{1}, 
...
\address{HCMC University of Technology and Engineering, Faculty of IT, Ho Chi Minh City 700000, Vietnam}
...
% Revised: Removed duplicate TM equation (already defined in Section III-D as eq:totalmotion); now references it directly to avoid restating the formula.
The foundational bounding parameters of any gesture are its total spatial displacement and its temporal duration. Applying the Total Motion (TM) metric defined in Eq. \eqref{eq:totalmotion} to the normalized centroid trajectory $p_t = (x_t, y_t, z_t) \in \mathbb{R}^3$ of the dominant articulator---scaled relative to the signer's inter-shoulder Euclidean distance to guarantee invariance across diverse human subjects---yields a substantial empirical divergence between the two corpora.

% Added: Expanded interpretation with verified data and inserted missing figures.
Empirical extraction reveals that the overall VSL corpus records a Total Motion amplitude of 25.01, which is 2.07 $\times$ higher than ASL's 12.07. Fig. \ref{fig:box_tm} and Fig. \ref{fig:hist_tm} visually substantiate this extreme spatial footprint. The distributions indicate that VSL sequences are not merely uniformly longer; rather, they are heavily right-skewed by highly kinetic outlier glosses possessing substantially larger physical amplitudes compared to the typical range observed in conversational ASL.
...
% Revised: Removed duplicate velocity/acceleration equations (originally re-used labels eq:velocity and eq:acceleration already defined in Section III-D, causing a LaTeX duplicate-label conflict); now references the Methodology definitions directly.
Applying the first- and second-order derivatives $V_t$ and $A_t$ defined in Eq. \eqref{eq:velocity} and Eq. \eqref{eq:acceleration} to the extracted centroid trajectories, we aggregate the global kinetic metrics \texttt{mean\_velocity} ($\bar{V}$), \texttt{max\_velocity}, \texttt{velocity\_std} ($\sigma_v$), \texttt{mean\_acceleration}, and \texttt{acceleration\_std} ($\sigma_a$) for both corpora.
...
% Revised: Removed duplicate SR equation (already defined in Section III-D as eq:straightness); now references it directly instead of restating the formula under a new label (eq:sr2).
While Total Motion captures the raw energy expended, it fails to describe the geometric topology of the execution. An identical Total Motion value could theoretically represent a single, long linear sweep or a tight, rapidly oscillating circle. To resolve this geometric ambiguity, we apply the Straightness Ratio (SR) \cite{pearson1901lines} defined in Eq. \eqref{eq:straightness}, bounded such that $0 \le \text{SR} \le 1$: a value approaching $1.0$ indicates a purely linear stroke, while a value approaching $0.0$ indicates a path that is increasingly curved, repeated, or recursive.
...
\textbf{Variance} \\
\hline
\emph{e then} (shy) & 3 & 114.390\\
\hline
\emph{quan bo} (jeans) & 3 & 46.700\\
\hline
\emph{man} (mosquito net) & 3 & 39.860 \\
\hline
\emph{chua bai} (correct exercise) & 3 & 4.596\\
\hline
\emph{con kien} (ant) & 3 & 4.521\\
\hline
\emph{hop nhom} (group meeting) & 3 & 4.495\\
\hline
...
\textbf{Metric} & \textbf{VSL Corpus} & \textbf{WLASL Benchmark} \\
\hline
Blur Mean (Sharpness) & 450.41 & 495.27 \\
Brightness Stability (Std) & 24.86 & 51.47 \\
Edge Density (Background) & 0.026 & 0.035 \\
Camera Frontality Score & 0.954 & 0.663 \\
Person Area Ratio & 0.341 & 0.778 \\
Body Occlusion Rate & 0.468 & 0.415 \\
Motion Magnitude & 0.229 & 0.414 \\
Hand Visibility Ratio & 0.998 & 0.996 \\
\hline
\end{tabular}
\label{tab:quality}
\end{table}
...
\centering
\includegraphics[width=0.98\linewidth]{../ACCESS_latex_template_20260513_V2/figures/wlasl_pca.png}\\[6pt]
\centerline{\footnotesize (a) WLASL PCA}
\vspace{8pt}
\includegraphics[width=0.98\linewidth]{../ACCESS_latex_template_20260513_V2/figures/vsl_pca.png}\\[6pt]
\centerline{\footnotesize (b) VSL PCA}
\end{minipage}
...
\begin{minipage}{0.75\linewidth}
\centering
\includegraphics[width=0.98\linewidth]{../ACCESS_latex_template_20260513_V2/figures/wlasl_tsne.png}\\[6pt]
\centerline{\footnotesize (a) WLASL t-SNE}
\vspace{8pt}
\includegraphics[width=0.98\linewidth]{../ACCESS_latex_template_20260513_V2/figures/vsl_tsne.png}\\[6pt]
\centerline{\footnotesize (b) VSL t-SNE}
\end{minipage}
...
\begin{minipage}{0.75\linewidth}
\centering
\includegraphics[width=0.98\linewidth]{../ACCESS_latex_template_20260513_V2/figures/wlasl_umap.png}\\[6pt]
\centerline{\footnotesize (a) WLASL UMAP}
\vspace{8pt}
\includegraphics[width=0.98\linewidth]{../ACCESS_latex_template_20260513_V2/figures/vsl_umap.png}\\[6pt]
\centerline{\footnotesize (b) VSL UMAP}
\end{minipage}
...
\footnotesize
\textit{Note:} Complexity Score denotes the normalized composite kinematic-complexity index derived from the motion metrics described in Section~\ref{sec:kinematics}. All reported values are directly computed from the extracted landmark trajectories.
...
A. Sams, A. H. Akash, and S. M. M. Rahman, ``SignBD-Word: Video-Based Bangla Word-Level Sign Language and Pose Translation,'' in \emph{Proc. 2023 14th Int. Conf. Computing Communication and Networking Technologies (ICCCNT)}, Delhi, India, 2023, pp. 1--7.

\bibitem{rubaiyeat2025bdslw60}
H. A. Rubaiyeat, H. Mahmud, A. Habib, and M. K. Hasan, ``BdSLW60: A word-level Bangla sign language dataset,'' \emph{Multimedia Tools and Applications}, vol. 84, pp. 42399--42423, 2025.
...
\begin{IEEEbiographynophoto}{Trinh Van Nguyen}
is currently pursuing the B.Eng. degree in Data Engineering with the Ho Chi
Minh City University of Technology and Education (HCMUTE), Ho Chi Minh City,
Vietnam. His undergraduate studies focus on data engineering, machine learning,
computer vision, and artificial intelligence.

His research interests include sign language recognition, computer vision, deep
learning, human pose estimation, multimodal learning, feature representation
learning, and data engineering. His current research focuses on the structural,
kinematic, and semantic analysis of word-level sign language datasets, with
applications to isolated sign language recognition and cross-dataset
evaluation.
\end{IEEEbiographynophoto}

\vspace{-8pt}

\begin{IEEEbiographynophoto}{Luong Thien Phuc}
is currently pursuing the B.Eng. degree in Data Engineering with the Ho Chi Minh
City University of Technology and Education (HCMUTE), Ho Chi Minh City,
Vietnam.

His research interests include computer vision, pattern recognition, deep
learning, and data analytics. He is particularly interested in visual
representation learning, human motion analysis, and sign language recognition
...
\begin{IEEEbiographynophoto}{Nguyen Ngoc Hai}
is currently pursuing the B.Eng. degree in Data Engineering with the Ho Chi Minh
City University of Technology and Education (HCMUTE), Ho Chi Minh City,
Vietnam.

His research interests include machine learning, computer vision, artificial
intelligence, and data engineering. His current work focuses on sign language
datasets, visual feature learning, and recognition systems for assistive
technologies.
\end{IEEEbiographynophoto}

\vspace{-8pt}

\begin{IEEEbiographynophoto}{Huynh Bao Minh}
is currently pursuing the B.Eng. degree in Data Engineering with the Ho Chi Minh
City University of Technology and Education (HCMUTE), Ho Chi Minh City,
Vietnam.

His research interests include deep learning, computer vision, image
processing, and intelligent systems. His current research focuses on sign
language recognition, multimodal data analysis, and human-centered artificial
intelligence.
\end{IEEEbiographynophoto}

\vspace{-8pt}

\begin{IEEEbiographynophoto}{Vo Le Minh Chien}
is currently pursuing the B.Eng. degree in Data Engineering with the Ho Chi Minh
City University of Technology and Education (HCMUTE), Ho Chi Minh City,
Vietnam.

His research interests include machine learning, computer vision, feature
engineering, and data mining. He is currently involved in research on sign
language recognition and visual understanding for intelligent systems.
\end{IEEEbiographynophoto}
