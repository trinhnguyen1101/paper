...
Empirical extraction reveals a substantial divergence: the overall VSL corpus records a Total Motion amplitude of 25.01, which is 2.07 $\times$ higher than ASL's 12.07. Fig. \ref{fig:box_tm} and Fig. \ref{fig:hist_tm} visually substantiate this extreme spatial footprint. The distributions demonstrate that VSL sequences are not merely uniformly longer; rather, they are heavily right-skewed by highly kinetic outlier glosses possessing immense physical amplitudes that far exceed the normal bounds of conversational ASL.
...
The compounding effects of extreme Total Motion, high execution velocity, and chaotic inter-signer variance are associated with a critical failure point in modern SLR: the missing landmark bottleneck. Pre-trained holistic estimators are vulnerable to motion blur and rapid self-occlusions characteristic of grassroots sign language datasets.

% Corrected & Verified: Re-verified percentages from keypoint_quality summaries.
Our automated tracking audit explicitly quantifies this degradation, documenting substantial hand-landmark missing rates reaching \textbf{39.6\% in ASL} and \textbf{32.3\% in VSL} under real-world kinematic stress. This indicates that for nearly one-third of the total video duration, the network receives little to no structural data regarding the primary articulators of the language.

This finding highlights a limitation in the contemporary deployment of Spatial-Temporal Graph Convolutional Networks (ST-GCN) and Vision Transformers. The standard programmatic convention of zero-padding missing $(x,y,z)$ coordinates disrupts the temporal continuity of the input graph. When a hand coordinate instantly drops to $(0,0,0)$ due to motion blur, the network's derivative functions register a massive acceleration spike, impairing the model's ability to learn meaningful spatial-temporal convolutions. Therefore, developing architectures that natively incorporate missingness-aware attention masking and robust trajectory interpolation represents a key unresolved challenge in bridging the gap between theoretical algorithm design and functional, low-resource deployment.
