...
\PARstart{T}{he} rapid progression of Sign Language Recognition (SLR) has been historically driven by the availability of large-scale, well-curated datasets \cite{b1}. High-resource languages, such as American Sign Language (ASL), benefit from mature structural benchmarks that enable the training of robust, generalized deep learning models. Conversely, regional and grassroots sign languages face extreme data sparsity, which fundamentally limits the generalization capabilities of these modern architectures. The scientific community has largely optimized algorithms for pristine, high-density environments, creating a significant research gap when these models are deployed on localized, low-resource languages. Understanding the precise mathematical and structural breaking points of these models requires a direct contrast between an established benchmark and a fundamentally constrained dataset.

This paper provides a rigorous structural, kinematic, and semantic analysis of a newly aggregated Vietnamese Sign Language (VSL) corpus directly compared against the benchmark Word-Level ASL (WLASL) dataset. Comparing VSL to WLASL is scientifically critical because it juxtaposes the theoretical ideal of SLR data—dense multi-view recordings and reliable cross-signer validation \cite{b1}—against the harsh reality of grassroots data collection. Emerging regional datasets, particularly in developing nations, often suffer from single-signer bias, acute class imbalance, and extreme motion blur resulting from consumer-grade camera sensors. For example, while the external VSL400 benchmark purports to contain 74,259 clips distributed across 400 gloss labels, real-world grassroots corpora aggregated from localized educational sources often reflect drastically sparser conditions that defy standard batch-training paradigms.

Kinematically, regional sign variations introduce structural complexities that heavily penalize standard spatial feature extractors. While holistic pose detection achieves $>99\%$ success rates across both datasets evaluated in this study, facial landmark detection exhibits unexpected cross-corpus vulnerability, achieving $99.0\%$ success in VSL but dropping to $96.2\%$ in ASL. Additionally, VSL signing introduces a spatial-temporal footprint fundamentally distinct from ASL. The VSL data exhibits a Total Motion amplitude of 25.01—a staggering 2.07 $\times$ higher than ASL's 12.07—and operates at a 1.51 $\times$ faster Mean Velocity. This high-amplitude, erratic signing directly degrades tracking accuracy, precipitating severe hand-tracking failures that result in a $32.3\%$ missing landmark rate across the VSL corpus.
...
We analyze temporal and spatial motion characteristics and demonstrate that VSL exhibits substantially more complex signing trajectories, including a 71.5\% reduction in trajectory linearity relative to WLASL.
...
The trajectory of Automatic Sign Language Recognition (SLR) research has historically been dictated by the availability, scale, and structural composition of the underlying datasets \cite{rastgoo2021sign}. The transition from handcrafted feature extraction mechanisms and Hidden Markov Models (HMMs) to deep spatial-temporal neural networks—such as Spatial-Temporal Graph Convolutional Networks (ST-GCN) \cite{jiang2021skeleton} and Vision Transformers \cite{camgoz2020sign}—was directly catalyzed by the curation of large-scale video corpora. However, the paradigm of dataset construction has heavily favored well-resourced languages, particularly American Sign Language (ASL), recorded under controlled conditions. This section exhaustively reviews the evolution of isolated word-level SLR datasets, explores the proliferation of multi-modal and non-English corpora, demarcates the boundary between isolated and continuous recognition benchmarks, and critically examines the deep learning architectures that have evolved to process these diverse linguistic domains.
...
To address the limitations of scale and environmental variance, researchers transitioned toward crowd-sourced and web-scraped methodologies. MS-ASL \cite{joze2019msasl} emerged as a pivotal large-scale dataset, significantly expanding the vocabulary size and introducing real-world background noise by aggregating data from online video platforms. This methodological shift laid the groundwork for the Word-Level American Sign Language dataset (WLASL) \cite{li2020wlasl}, which currently reigns as the dominant, \emph{de facto} standard benchmark for isolated ASL recognition. WLASL aggregates over 21,000 video clips across a massive 2,000-gloss vocabulary, featuring 119 distinct signers. In our empirical comparisons, a 11,980-video subset of WLASL serves as the primary baseline.

The structural composition of WLASL is defined by its relatively dense and stable sampling distribution. With a median of six samples per class and a highly consistent frame rate, WLASL provides an adequate statistical manifold for training deep 3D Convolutional Neural Networks (3D-CNNs), such as Inflated 3D ConvNets (I3D) \cite{carreira2017quo}, and hybrid Convolutional Long Short-Term Memory (CNN-LSTM) networks \cite{hochreiter1997long}. The initial WLASL benchmark established Pose-based Temporal Graph Convolution (Pose-TGCN) as a robust baseline, fundamentally proving that extracting skeletal coordinates and modeling their spatial-temporal dynamics yields superior generalization compared to raw RGB pixel processing. Nevertheless, a critical research gap remains: WLASL is structurally constrained by its English-centric linguistic roots and its reliance on relatively linear, well-framed ASL trajectories. The widespread adoption of WLASL has inadvertently caused the SLR community to over-optimize neural architectures for ASL-specific kinematic amplitudes and sequence lengths, creating severe domain shift vulnerabilities when these models are applied to globally disparate, highly curved sign languages.
...
For Turkish Sign Language, Sincan and Keles introduced AUTSL \cite{sincan2020autsl}, a large-scale, multi-modal dataset comprising 38,336 isolated sign videos across 226 glosses, performed by 43 different signers. A distinguishing feature of AUTSL is its utilization of Microsoft Kinect sensors to capture both RGB and Depth (RGB-D) modalities simultaneously. By providing depth maps alongside high-resolution spatial coordinates, AUTSL facilitated the development of advanced 3D ResNet \cite{he2016deep} and multi-stream CNN-LSTM architectures capable of resolving severe self-occlusion—a persistent bottleneck in 2D skeletal tracking. Despite its technological sophistication, AUTSL's tightly bounded vocabulary of 226 glosses limits its applicability for broad, large-vocabulary translation tasks, maintaining a highly dense sampling rate that does not accurately reflect the sparsity of in-the-wild data collection.

Similar densely resourced initiatives have been executed for Indian and Bangla Sign Languages. The INCLUDE dataset \cite{sridhar2020include} provides 4,287 RGB videos covering 263 Indian Sign Language glosses, offering a vital resource for South Asian SLR. However, the most aggressive scaling of non-English isolated SLR is observed in the recent lineage of Bangla Sign Language (BdSL) datasets. Early efforts, such as SignBD \cite{sams2023signbd}, provided a modest 6,000 videos across 200 signs using 16 signers. This was rapidly superseded by BdSLW60 \cite{rubaiyeat2025bdslw60}, which utilized MediaPipe Holistic landmarks to train attention-based Bi-LSTM networks across 9,307 videos for 60 classes. While these datasets successfully diversify the linguistic pool, a fundamental gap persists: they uniformly rely on dense per-class distributions engineered within controlled settings, failing to provide a realistic testing ground for few-shot learning and long-tail imbalances.

\subsection{The Vietnamese Sign Language Context and Grassroots Sparsity}
Vietnamese Sign Language (VSL) has remained conspicuously absent from the upper echelons of large-scale, deep learning-driven SLR benchmarks. VSL possesses a highly unique topological execution space, characterized by expansive motion amplitudes, prolonged temporal sequence durations, and heavy reliance on rapid, curved trajectories that frequently trigger motion blur and tracking failures in standard pose estimators.

To bridge this geographical and linguistic gap, Nguyen Quoc et al. recently introduced the VSL400 benchmark \cite{nguyen2026vsl400}. VSL400 is a meticulously curated, high-quality resource designed for reproducible word-level SLR. It encompasses 74,259 multi-view clips across 400 manually annotated glosses, recorded by 28 signers across diverse age and gender demographics. Recorded in a controlled studio environment with synchronized frontal, left, and right camera views, VSL400 achieves a formidable sampling density of approximately 185.6 videos per gloss. This density ensures a balanced class distribution, effectively shielding standard supervised classifiers from the destabilizing effects of long-tail imbalances. The VSL400 dataset provides a vital, pristine baseline for evaluating signer-invariant modeling and multi-view feature fusion in underrepresented sign languages.

However, it is scientifically imperative to explicitly delineate the VSL400 benchmark from the primary subject of this manuscript. \textbf{Our study does not analyze the VSL400 dataset; rather, we systematically analyze a custom-aggregated, high-variance VSL corpus consisting of 3,315 distinct classes but containing only 4,362 total videos.} This intentional selection represents a fundamental divergence in dataset curation philosophy. While VSL400 maximizes per-class density at the expense of lexical breadth (capping at 400 glosses), our custom VSL corpus trades sampling density for an eight-fold greater lexical coverage (3,315 glosses). This trade-off perfectly mirrors the harsh, unstructured realities of grassroots, low-resource sign language data collection. In real-world assistive technology deployment, developers rarely have access to 185 pristine, multi-view samples per word. Instead, they must construct models from highly fragmented, crowd-sourced data where the median sample count is exactly one. By confronting this extreme few-shot regime—averaging 1.32 samples per class and exhibiting a severe Gini coefficient imbalance of 0.203—our study explicitly interrogates the exact conditions under which modern, data-hungry architectures fail. This corpus provides the ultimate stress test for cross-lingual transfer learning, exposing the critical research gap of how algorithms trained on the densely populated, linearly executed WLASL benchmark react when exposed to the highly curved, extremely sparse, and structurally divergent kinematics of grassroots VSL.
...
An alternative strategy to manual annotation is the weak-supervision scaling approach demonstrated by BSL-1K \cite{albanie2020bsl1k}. By leveraging mouthing cues and corresponding broadcast subtitles, Albanie et al. automatically annotated a massive British Sign Language corpus comprising roughly 1,000 hours of video and 1,000 glosses with minimal human intervention. While continuous datasets like PHOENIX, How2Sign, and BSL-1K effectively capture the natural flow of sign language, the immense temporal complexity and alignment ambiguity inherent in sentence-level data make them unsuitable for the precise, frame-level kinematic profiling and direct cross-lingual Dynamic Time Warping (DTW) feature-space mapping conducted in our study. Consequently, our comparative structural analysis is strictly anchored to isolated-word corpora, where the absolute physical boundaries of individual semantic concepts can be mathematically isolated, standardized, and compared across the VSL and ASL linguistic domains.

\subsection{Algorithmic Advancements and the Missing-Data Bottleneck}
The architectural evolution of SLR systems is fundamentally intertwined with the structural characteristics of the datasets they process. Early benchmarks relied on bounding-box crops and raw pixel processing, utilizing 3D-CNNs to extract spatial-temporal features \cite{li2020wlasl}. While effective in highly controlled environments, pixel-based approaches suffer from extreme computational overhead and are highly susceptible to background noise, clothing variations, and illumination changes.

To mitigate these issues, the field has universally adopted skeletal representation modeling \cite{yin2020signbert}. By extracting topological joint coordinates via pose estimators like MediaPipe Holistic \cite{lugaresi2019mediapipe}, researchers map the human body as a non-Euclidean graph. Spatial-Temporal Graph Convolutional Networks (ST-GCN) \cite{stgcn2018} and their variants, such as Two-Stream Adaptive GCNs \cite{shi2019two} and Disentangled GCNs \cite{liu2020disentangling}, have become the premier architectures for processing these skeletal inputs. They utilize message-passing algorithms to learn the biomechanical correlations between adjacent joints (e.g., wrist to elbow) over time.

Despite the mathematical elegance of ST-GCNs and Vision Transformers \cite{camgoz2020sign}, their empirical success is predicated on the assumption of complete, unbroken data manifolds. A critical research gap remains largely ignored in the literature: the ``missing-data bottleneck.'' Pre-trained estimators like MediaPipe are highly vulnerable to the motion blur, self-occlusion, and erratic trajectories inherent in high-speed, grassroots signing. As our subsequent analysis reveals, hand-landmark missing rates reach catastrophic levels (39.6$\%$ in ASL and 32.3$\%$ in VSL) under real-world kinematic stress. Standard architectural practices of zero-padding these missing tensors violently corrupt the temporal continuity of the input graphs, destroying the model's ability to learn meaningful spatial-temporal convolutions. Designing architectures that natively incorporate missingness-aware attention masking and robust trajectory interpolation represents the paramount unresolved challenge in bridging the gap between theoretical algorithm design and functional, low-resource deployment.

\section{Mathematical Foundations and Methodology} 
\label{sec:methodology}
To rigorously quantify the structural, kinematic, and semantic divergences between Vietnamese Sign Language (VSL) and American Sign Language (ASL), we designed an exhaustive, fully automated computational pipeline. This framework transitions raw, unstructured RGB video data into a high-dimensional, standardized latent space, enabling precise mathematical comparisons across highly asynchronous linguistic domains. This section meticulously details the dataset curation protocols, the skeletal graph extraction pipeline, the 82-dimensional feature extraction schema, the kinematic formulations, Dynamic Time Warping (DTW) algorithms, and the underlying statistical bounding mechanisms.
...
A foundational objective of this study is mapping the feature-space semantic confusability between VSL and ASL. Direct Euclidean comparison of feature vectors is mathematically invalid because sign executions possess inherently unequal temporal lengths ($N \neq M$) and are subject to non-linear temporal warping (e.g., a signer hesitating at the apex of a gesture). 
...
The empirical efficacy of deep neural networks relies intrinsically on the structural density, scale, and distributional equality of their training manifolds. In real-world sign language recognition, the transition from highly controlled, studio-recorded environments to grassroots, in-the-wild corpora inherently introduces extreme data sparsity. This section rigorously quantifies the morphological imbalance between the densely sampled Word-Level American Sign Language (WLASL) benchmark and our custom-aggregated Vietnamese Sign Language (VSL) corpus, subsequently detailing the catastrophic implications of these distributions on modern deep learning paradigms.
...
As summarized in Table \ref{tab:1}, the VSL corpus contains 1.66 $\times$ more distinct semantic glosses than WLASL, yet comprises 63.6\% fewer total video instances. This creates an environment of extreme sparsity. The WLASL dataset maintains a robust mean of 5.99 samples per class, with a median of 6.00 and a maximum class count of 16. This provides a sufficiently dense manifold for standard batch optimization. In stark contrast, the VSL dataset is defined by extreme sparsity. It exhibits a mean of exactly 1.32 samples per class, a collapsed median of 1.00, and a maximum class count of merely 6. Alarmingly, our analysis reveals that 99.97\% of VSL classes contain fewer than five samples, and thousands of classes consist of a single video exemplar, as shown in Fig. \ref{fig:1}.
...
% Added: Standard deviation mathematically proves the lack of variance in VSL.
...
This distribution contradicts standard long-tail phenomena often observed in computer vision benchmarks \cite{buda2018systematic}, \cite{lin2017focal}. The VSL dataset does not simply feature a long, tapering right tail; instead, it is defined by \textit{floor-dominated sparsity}. With a Coefficient of Variation (CV) of 0.549 (significantly higher than WLASL's 0.325), VSL's inequality is driven by the fact that the vast majority of its 3,314 classes are locked at the absolute theoretical floor of 1 or 2 samples, as corroborated by Table \ref{tab:1}. Furthermore, the top 10\% most frequent classes in the VSL corpus monopolize precisely 23.0\% of the dataset volume, whereas the top 10\% in WLASL capture only 16.9\%. Conversely, the bottom 10\% of VSL classes account for a mere 7.6\% of the total data.

Fig. \ref{fig:2} visually corroborates this mathematical output via the Lorenz curve, plotting cumulative video instances against cumulative class proportions to expose the severe sample inequality represented by the Gini coefficient of 0.203. Fig. \ref{fig:3} further illustrates the collapsed median of the VSL corpus relative to the WLASL dataset.

\subsection{Architectural Implications}
% Revised: Restructured for clarity and expanded to include standard deviation impacts.
The quantitative extraction of these imbalance metrics is not merely a statistical exercise; these distributions actively dictate the failure modes of modern deep learning architectures \cite{cui2019class}. The interaction between extreme few-shot environments, floor-dominated sparsity, and specific neural typologies manifests in several catastrophic ways:

\subsubsection{Impact on Few-Shot and Long-Tail Learning Dynamics}
Traditional supervised learning relies on Empirical Risk Minimization (ERM) driven by Softmax cross-entropy loss. In a long-tail distribution with a Gini coefficient of 0.203, ERM intrinsically favors majority classes to rapidly minimize global loss, starving the ``tail'' classes of gradient updates. However, because VSL's tail consists of 99.97\% of the dataset operating under $\le 5$ samples, the entire network collapses into an underdetermined state. 

% Added: Interpretation of standard deviation constraints on gradient descent.
In an extreme few-shot regime (where the median $n=1$ and standard deviation $\sigma=0.72$), a model has absolutely no mathematical basis to compute intra-class variance. Unlike WLASL, where a broader sample distribution ($\sigma=1.95$) provides diverse execution angles, VSL's severe sparsity forces the neural network to strictly memorize the single signer's environment, clothing, and coordinate scaling. Consequently, it becomes mathematically impossible to decouple the invariant semantic core of the sign from the idiosyncratic, sample-specific noise, permanently paralyzing cross-signer generalization.
...
The empirical disparity in dataset scale is compounded by profound structural differences in how the signs themselves are physically executed. To rigorously map these differences, the spatial-temporal coordinates of both corpora were processed through our kinematic feature extraction pipeline, detailing a fundamental divergence in motion geometry.
...
Empirical extraction reveals a staggering divergence: the overall VSL corpus records a Total Motion amplitude of 25.01, which is 2.07 $\times$ higher than ASL's 12.07. Fig. \ref{fig:box_tm} and Fig. \ref{fig:hist_tm} visually substantiate this extreme spatial footprint. The distributions demonstrate that VSL sequences are not merely uniformly longer; rather, they are heavily right-skewed by highly kinetic outlier glosses possessing immense physical amplitudes that far exceed the normal bounds of conversational ASL.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/boxplot_total_motion.png}
\caption{\textbf{Total Motion Distribution Comparison.} Boxplots illustrating the massively expanded trajectory amplitude of VSL compared to the tightly bounded ASL baseline.}
\label{fig:box_tm}
\end{figure}
...
\caption{\textbf{Total Motion Density Histogram.} The VSL corpus exhibits severe right-skewed behavior, heavily populated by extreme outlier glosses demanding high physical exertion.}
\label{fig:hist_tm}
\end{figure}
...
\caption{\textbf{Mean Velocity Comparison.} The grassroots VSL data demonstrates significantly higher execution speeds, challenging temporal alignment boundaries.}
\label{fig:box_vel}
\end{figure}
...
\caption{\textbf{Mean Acceleration Comparison.} Higher acceleration variances in VSL highlight an aggressive, disjointed execution style lacking the smooth transitions typical of benchmark corpora.}
\label{fig:box_acc}
\end{figure}

% Revised: Kept original paragraph and appended explicit causal links to verified missing hand rates.
High values of $\sigma_a$ indicate aggressive, highly volatile ballistic strokes. In computer vision paradigms, excessive velocity and high-variance acceleration immediately induce severe motion blur. Because shutter exposure times in generic web-cameras remain static, the rapid ballistic strokes characteristic of VSL artificially smear the RGB pixel boundaries of the hand. This phenomenon directly degrades landmark confidence scores across the skeletal graph and precipitates the severe 32.3\% missing hand tracking rate observed in the VSL data, crippling standard 2D tracking pipelines.

\subsubsection{Curvature, Straightness, and Trajectory Entropy}
% Revised: Kept original paragraph verbatim.
While Total Motion captures the raw energy expended, it fails to describe the geometric topology of the execution. An identical Total Motion value could theoretically represent a single, long linear sweep or a tight, violently vibrating circle. To resolve this geometric ambiguity, we compute the Straightness Ratio (SR) \cite{pearson1901lines}, defined as the quotient of the net Euclidean displacement from the initial to the terminal frame versus the total trajectory path length: 
\begin{equation} \label{eq:sr2} 
\text{SR} = \frac{\lVert p_T - p_1 \rVert_2}{\text{TM}}, \qquad 0 \le \text{SR} \le 1 
\end{equation}
A Straightness Ratio approaching $1.0$ mathematically guarantees a purely linear stroke. Conversely, as SR approaches $0.0$, the path becomes increasingly curved, repeated, or recursive.

% Added: Integration of the SR boxplot and visual trajectory examples to ground the math.
Analysis of the extracted features demonstrates that VSL trajectories are highly curved, exhibiting an SR of 0.039 compared to ASL's 0.137. This mathematically equates to a 71.5\% reduction in trajectory linearity. Fig. \ref{fig:box_sr} quantifies this systemic statistical collapse toward zero. To physically ground these mathematical abstractions, Fig. \ref{fig:traj_vsl} and Fig. \ref{fig:traj_asl} visualize the spatial coordinates of representative signs from both corpora. The VSL trajectories display profound spatial turbulence and recursive curvature, standing in stark contrast to the smoother, more deterministic geometric boundaries of ASL.
...
\caption{\textbf{VSL Spatial Signatures.} Physical 2D trajectory visualizations of VSL signs, demonstrating severe spatial turbulence, recursive wrapping, and compounding direction changes.}
\label{fig:traj_vsl}
\end{figure}
...
The motion variance (MV), derived from $\sigma_v^2$, effectively quantifies the erraticness or temporal inconsistency of the sign's execution. Compounding its lack of straightness, VSL yields an extreme 67.25 direction changes per sequence (compared to ASL's 40.79) and a Motion Variance of 0.122 (2.17 $\times$ higher than ASL's 0.056). Fig. \ref{fig:scatter_tm_mv} maps the critical correlation between Total Motion and Motion Variance, revealing that as VSL spatial trajectories lengthen, their execution entropy scales exponentially, heavily compounding the difficulty of sequence prediction.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/scatter_total_motion_vs_motion_variance.png}
\caption{\textbf{Kinematic Entropy Scaling.} Scatter plot mapping the exponential increase of motion variance relative to total trajectory amplitude, underscoring the instability of highly active VSL glosses.}
...
However, in VSL, the extreme combination of high velocity, low straightness (0.039), and massive kinematic entropy (67.25 direction changes) forces the skeletal graph to violently fold back upon itself. Because the hands repeatedly occupy the exact same $(x, y)$ coordinate space across different temporal timestamps, 2D graph convolutions suffer from severe aliasing; the network cannot distinguish whether the hand is moving forward along the Z-axis or merely returning to a previous anchor point. 

% Added: Deepened scientific implication for GNN engineering.
This structural folding fundamentally corrupts the message-passing efficiency of standard Spatial-Temporal Graph Convolutional Networks (ST-GCNs). Without explicit temporal masking mechanisms or depth-aware 3D coordinate unrolling, models trained on datasets possessing such extreme kinematic profiles will inherently fail to converge. Instead of isolating invariant linguistic features, the network inadvertently memorizes idiosyncratic spatial noise, cementing the necessity for domain-aware architectural adaptations when operating in low-resource environments.
...
Specifically, VSL exhibits a demonstrably higher reliance on bi-manual execution. Analysis of the extracted dataset metrics proves that 72.8\% of VSL sequences are classified as two-handed operations, compared to only 64.7\% in ASL. Fig. \ref{fig:asl_hands} and Fig. \ref{fig:vsl_hands} illustrate these divergent hand-usage distributions. This increased prevalence of dual-hand tracking in VSL intrinsically multiplies the probability of occlusions, self-intersections, and crossing feature trajectories. From a computational perspective, this demands highly sophisticated spatial attention mechanisms within the model architecture to prevent feature collapse when the left and right hand coordinates temporarily merge in 2D projection space.
...
\caption{\textbf{VSL Hand Usage Distribution.} Distribution of hand usage in the VSL corpus, demonstrating a heavy skew toward two-handed signs (72.8\%), which exponentially increases the risk of spatial occlusion during tracking.}
\label{fig:vsl_hands}
\end{figure}

% Revised: Expanded into Biomechanics and Linguistic Implications.
\subsubsection{Biomechanics and Linguistic Implications}
Linguistically, strict adherence to these 3D spatial vectors is mathematically required to encode deixis, referential loci, and cultural syntactic markers. Our extraction of 3D orientation tensors (Finger Direction and Palm Normal) from the dataset reveals that Vietnamese Sign Language (VSL) enforces highly rigid biomechanical protocols for specific semantic classes. 
...
The empirical results are striking, revealing that execution consistency is highly dependent on the semantic nature of the sign. While highly standardized, objective signs exhibit near-perfect kinematic consistency, complex verbs or emotionally driven expressions fluctuate wildly in their spatial boundaries. 
...
% Added: Explicitly addressed the missing landmark bottleneck identified in the audit.
\subsection{The Missing Landmark Bottleneck}
\label{sec:missing_landmarks}

The compounding effects of extreme Total Motion, high execution velocity, and chaotic inter-signer variance directly precipitate the most critical failure point in modern SLR: the missing landmark bottleneck. Pre-trained holistic estimators are fundamentally vulnerable to the severe motion blur and rapid self-occlusions characteristic of grassroots sign language datasets.

% Corrected & Verified: Re-verified percentages from keypoint_quality summaries.
Our automated tracking audit explicitly quantifies this degradation, documenting catastrophic hand-landmark missing rates reaching \textbf{39.6\% in ASL} and \textbf{32.3\% in VSL} under real-world kinematic stress. This indicates that for nearly one-third of the total video duration, the neural network receives absolutely no structural data regarding the primary articulators of the language.

This finding exposes a fatal flaw in the contemporary deployment of Spatial-Temporal Graph Convolutional Networks (ST-GCN) and Vision Transformers. The standard programmatic convention of zero-padding missing $(x,y,z)$ coordinates violently corrupts the temporal continuity of the input graph. When a hand coordinate instantly drops to $(0,0,0)$ due to motion blur, the network's derivative functions register an infinite acceleration spike, destroying the model's ability to learn meaningful spatial-temporal convolutions. Therefore, developing architectures that natively incorporate missingness-aware attention masking and robust trajectory interpolation represents the paramount unresolved challenge in bridging the gap between theoretical algorithm design and functional, low-resource deployment.
...
The fundamental bottleneck in scaling Sign Language Recognition (SLR) systems globally is the assumption of cross-lingual structural universality. When deep neural networks pretrained on the Word-Level American Sign Language (WLASL) benchmark are deployed on grassroots datasets like the Vietnamese Sign Language (VSL) corpus, they encounter a severe, undocumented semantic domain shift. To mathematically map this confusability space, we transition from analyzing isolated physical kinematics to computing a direct, cross-lingual feature-space similarity map.
...
\end{equation} 
A Similarity Score approaching $1.0$ indicates that the VSL and ASL glosses share near-identical structural and kinematic topologies. A score trending toward $0.0$ implies a complete semantic and topological disconnect.

% Added: Restored the omitted distribution visualization of these similarity scores.
As illustrated in Fig. \ref{fig:sim_hist}, plotting the aggregated 82 kinematic and structural features yields similarity scores that are widely dispersed from 0.1452 to 0.9742. This wide dispersion proves unequivocally that cross-lingual semantic overlap is not uniform, necessitating individual gloss-level analysis.
...
The extreme variance in these similarity scores mathematically isolates the boundaries of cross-lingual transferability. Highly iconic, visually grounded signs exhibit near-perfect semantic overlap. Concepts anchored to absolute physiological landmarks—such as ``bụng'' (abdomen)—achieve a similarity score of 0.9742. Similarly, universally standardized manual alphabetic equivalents yield exceptionally high similarity scores (e.g., ``i'' scoring 0.9674, ``a'' scoring 0.9634). These highly overlapping clusters represent a stable, universal latent space where zero-shot transfer learning can be successfully deployed without encountering domain shift.

Conversely, culturally specific concepts exhibit entirely divergent topological structures. The mapping for ``phòng thư viện'' (library) scores the lowest at 0.1452, and the mapping for ``Mexico'' scores 0.1829. 
...
This quantifiable semantic gap proves unequivocally that ASL and VSL cannot be naively merged into a single latent space without sophisticated, targeted domain adaptation. 

\subsection{Domain Shift and Hard Negative Mining} 
The existence of this vast confusability space dictates that simply aggregating ASL and VSL datasets into a unified training corpus will induce a catastrophic domain shift. If a Vision Transformer or ST-GCN is naively fed both the ASL and VSL versions of ``library'' under the identical supervised label, the loss function will attempt to reconcile two completely orthogonal skeletal graphs. The resulting gradient collision will force the model to collapse to a degenerate, average trajectory that matches neither language, destroying inference accuracy.

% Added: Restored the crucial regression plot linking kinematics to semantic divergence.
Crucially, this mapping degradation is directly governed by physical trajectory divergence. As demonstrated in Fig. \ref{fig:10}, there is a strict, near-perfect negative linear correlation between the assigned DTW similarity score and the absolute difference in gloss-level motion-complexity proxy vectors. 

\begin{figure}[htbp] 
\centering 
\includegraphics[width=\linewidth]{figures/08_similarity_vs_motion_difference.png} 
\caption{\textbf{Similarity vs. Kinematic Difference Correlation.} Scatter plot proving the strict negative linear correlation between cross-lingual DTW similarity scores and absolute differences in motion complexity. Glosses failing to map are mathematically isolated by their geometric boundaries.} 
\label{fig:10} 
\end{figure}

However, this divergence provides a highly potent mechanism for algorithmic optimization: \textbf{Hard Negative Mining} \cite{robinson2020contrastive}. In modern contrastive learning paradigms (e.g., InfoNCE loss) \cite{oord2018representation}, models learn robust embeddings by maximizing the distance between dissimilar classes. Traditionally, negatives are sampled randomly, which often pairs kinematically obvious differences (e.g., ``head'' vs. ``feet''), resulting in weak, uninformative gradient updates.
...
The visual and numerical evidence should be read together rather than independently. In the PCA views, both corpora form broad overlapping clouds with peripheral outliers, which suggests that the dominant variance directions are shared by multiple glosses rather than cleanly partitioned by class. The t-SNE and UMAP views reveal more structure: local pockets appear within each corpus, but the pockets are connected by bridges and interleaved with nearby classes. This is exactly the geometry that one expects when the descriptor space captures meaningful articulatory structure but the sample density is too low to resolve sharp class boundaries. In other words, the representations are informative, yet not sufficiently compressed into separable islands for naive linear discrimination.
...
The main limitation of the present evidence is scope. The current study characterizes dataset geometry, but it does not measure recognition accuracy under controlled training protocols. As a result, the findings should be interpreted as constraints on representation learning rather than as direct performance predictions. Another limitation is the reliance on monocular 3D pose estimation, which remains vulnerable to self-occlusion, camera angle, and fast motion. A final limitation is the manual nature of the cross-lingual alignment subset, which is necessarily incomplete and may overrepresent visually intuitive pairs while underrepresenting culturally complex mappings. These limitations do not weaken the core conclusion; they simply limit the evidence to descriptive and geometric analysis.

Taken together, the corpus properties identified here motivate a domain-aware training strategy: preserve missingness information, stratify by class support, exploit DTW-based hard negatives, and evaluate low-resource classes separately from dense ones. This is a methodological implication of the data, not a claim about any specific model's achieved accuracy. It is also the clearest route toward reducing the mismatch between the geometry of sign-language corpora and the assumptions baked into conventional supervised pipelines.

\section{Conclusion} \label{sec:conclusion}

This study presented a massive-scale structural, kinematic, and semantic comparative analysis of 16,342 video sequences, explicitly defining the domain shift between high-resource algorithmic benchmarks (WLASL) and extreme low-resource, grassroots corpora (custom VSL). Our empirical investigation characterizes the data conditions that currently limit cross-lingual transfer learning. We demonstrated that real-world VSL data is constrained by a floor-dominated long-tail imbalance, yielding a severe Gini coefficient of 0.203 and an average of merely 1.32 samples per class, contrasting sharply with the 5.99 average of the WLASL baseline. Kinematically, VSL execution demands significantly extended temporal receptive fields (92.48 mean frames), generates a 2.07 $\times$ higher total motion amplitude, and produces highly recursive, curved spatial trajectories (straightness ratio of 0.039) compared to ASL. Furthermore, our topological analysis revealed a heavy skew toward two-handed execution in VSL (72.8\%) relative to ASL (64.7\%), which intrinsically multiplies the probability of intersecting feature trajectories and spatial occlusion. 

The practical implications of these structural divergences suggest a shift in how spatial-temporal models are engineered for low-resource deployment. The compounding effects of high execution velocity and severe trajectory curvature directly precipitate catastrophic hand-landmark missing rates, which we verified to reach 39.6\% in ASL and 32.3\% in VSL. Consequently, the standard programmatic convention of zero-padding missing coordinates is mathematically destructive, as it injects infinite acceleration spikes into the network's derivative functions. Practitioners should move toward missingness-aware attention masking and relative-distance feature embeddings. Additionally, our cross-lingual Dynamic Time Warping (DTW) mapping proved that semantic transferability is highly volatile, with similarity scores ranging dramatically from 0.1452 to 0.9742. Because culturally divergent signs can induce substantial topological dissimilarity, developers cannot simply aggregate disparate sign languages into a unified latent space. Instead, models must leverage precise DTW confusability mappings to execute targeted hard-negative mining, explicitly forcing neural attention heads to distinguish between highly overlapping macro-trajectories and fine-grained terminal handshapes.

Future research should shift from broad dataset scaling toward variance-aware data curation and adaptive architectural design. Rather than uniformly scraping broad vocabularies to maximize class counts, dataset collection efforts should prioritize deep, multi-signer sampling specifically for highly entropic, emotionally complex glosses that exhibit massive inter-signer execution variance. Algorithmically, the discovery of severe spatial aliasing—driven by an extreme 67.25 direction changes per VSL sequence—suggests that rigid 2D Spatial-Temporal Graph Convolutional Networks (ST-GCNs) are nearing their theoretical limits for unconstrained grassroots data. Future architectures should explore elastic, warp-invariant temporal alignments and natively integrate robust 3D coordinate unrolling to resolve the ambiguity of orthogonal foreshortening. Ultimately, translating these empirical boundaries into missing-data-resilient neural architectures will help support more robust and equitable Sign Language Recognition systems globally.

\appendices

\section{VSL Kinematic Complexity Ranking}
As discussed in the main text, extreme kinematic complexity severely degrades holistic tracking stability, serving as a primary driver of the missing-landmark bottleneck. Table \ref{tab:app_kinematic} details the upper bounds of the VSL physical execution space. By incorporating direction changes and straightness ratios directly into the kinematic profile, we mathematically isolate how highly recursive, non-linear spatial boundaries compound raw velocity. Notably, the most extreme motion complexity scores are derived from highly fragile sample populations within the long-tail distribution, reiterating the absolute necessity for few-shot robustness in grassroots dataset processing.
...
Q. Nguyen \emph{et al.}, ``VSL400: A carefully curated word-level Vietnamese Sign Language dataset,'' \emph{IEEE Access}, vol. 14, pp. 3400--3415, 2026.
...
J. Forster \emph{et al.}, ``RWTH-PHOENIX-Weather: A large vocabulary sign language recognition and translation corpus,'' in \emph{Proc. Int. Conf. Lang. Resour. Eval. (LREC)}, Istanbul, Turkey, 2012, pp. 3285--3289.
...
A. Albanie \emph{et al.}, ``BSL-1K: Scaling up co-articulated sign language recognition using mouthing cues,'' in \emph{Proc. Eur. Conf. Comput. Vis. (ECCV)}, Glasgow, U.K., 2020, pp. 35--53.

\bibitem{yin2020signbert}
N. Yin \emph{et al.}, ``SignBERT: Pre-training of hand-model-aware representation for sign language recognition,'' in \emph{Proc. IEEE/CVF Int. Conf. Comput. Vis. (ICCV)}, Montreal, QC, USA, 2021, pp. 11087--11096.
